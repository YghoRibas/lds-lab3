...
export const AlunoForm = ({ id, modalId, refetchAlunos, onClose }: IProps) => {
  ...
  const formik = useFormik<IAluno>({
    ...
    validationSchema: Yup.object({
      ...
      moeda: Yup.number().required('Campo obrigatório'),
      ...
      <div className='modal'>
        <div className='modal-box max-h-[700px]'>
          <div className='flex justify-between items-center mb-6'>
            ...
            <label htmlFor={modalId} className='modal-close btn btn-ghost' onClick={() => formik.resetForm()}>
              <span>X</span>
            </label>
          </div>
          <form onSubmit={formik.handleSubmit}>
            <div className='flex flex-col gap-4 overflow-y-auto max-h-[460px] p-2'>
              ...
              {id && (
                <div className='form-control w-full'>
                  <input type='number' placeholder='Moeda' id='moeda' name='moeda' className={`input input-bordered w-full ${formik.errors.moedas && formik.touched.moedas && 'input-error'}`} value={formik.values.moedas} onChange={formik.handleChange} />
            ...
            <div className='modal-action'>
              {id && (
                <button className='btn btn-error' onClick={() => excluirAluno()}>
                  Excluir
                </button>
              )}
